...
docker run --detach \
           ...
           brazildatacube/sits-rstudio:1.2.0
```
...
To use the Jupyter Notebook environment with SITS-enabled, you can run a container from the [brazildatacube/sits-jupyter:1.2.0](https://hub.docker.com/r/brazildatacube/sits-jupyter) image, which is made available on DockerHub. For this, the following command can be used:

```shell
docker run --detach \
           --publish 127.0.0.1:8888:8888 \
           --name my-sits-jupyter \
           brazildatacube/sits-jupyter:1.2.0
```
...
- `-t`: SITS Tag version used in generated image (Default is `1.2.0`).
- `-p`: Image name prefix (Default is `bdc`).
- `-e`: SITS environment type (`full` or `minimal`. Default is `full`).
- `-h`: show a help message.

Below is an example of using the utility script. The tag name defined for the SITS image is `1.2.0`.

```shell
./build.sh -n -p brazildatacube -t 1.2.0
```
...
```
brazildatacube/sits-jupyter   1.2.0          9d1fe19659e5   3 hours ago    7.12GB
brazildatacube/sits-rstudio   1.2.0          3cea78fefed3   3 hours ago    7.11GB
brazildatacube/sits           1.2.0          3383b933965e   3 hours ago    6.37GB
brazildatacube/sits-r         1.2.0          c17f0fe43f07   3 hours ago    5.82GB
brazildatacube/sits-base      1.2.0          897aca0ceb1d   37 hours ago   3.56GB
